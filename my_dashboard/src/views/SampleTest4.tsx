...
function Detail({ tab }: { tab: Tab }) {
  if (tab === 'architecture') return <>
    ...
    <Card kicker="NOT DONE YET" title="Phase 4-6 남은 작업"><div className="check-list"><p><b>Phase 4 — DMA 엔진 (진행중)</b><span>dma_sched.sv 완료·검증. desc_fetch.sv는 작성 중(미검증). axi_rd_master, axi_wr_master, wr_track 아직 시작 전.</span></p><p><b>Phase 5 — top</b><span>irq_ctrl, perf_cnt, daq_subsystem.sv (모든 CDC를 명시적으로 인스턴스화)</span></p><p><b>Phase 6 — 통합</b><span>integration UVM, block별 formal, regression script</span></p></div></Card>
  </>
  ...
  if (tab === 'synthesis') return <>
    <Card kicker="ASIC FLOW — TOOLCHAIN" title="OpenLane2 + sky130A, WSL Ubuntu + Docker"><pre className="rtl-code">{`오픈소스 전용, foundry NDA 없음\nYosys (합성) -> OpenROAD (P&R, STA) -> Magic (DRC) -> Netgen (LVS) -> KLayout (스트림아웃)\n실행: openlane --docker-no-tty --dockerized <config.json>\nPDK: sky130A, volare로 첫 실행 시 자동 다운로드\n환경: WSL Ubuntu, Python 3.11 venv (uv로 생성), openlane==2.3.10`}</pre><p className="rtl-guide-note">FPGA(yosys+nextpnr, 이미 있는 oss-cad-suite로 PDK 없이 즉시 가능)와 실제 foundry PDK(접근 불가, NDA 필요) 사이의 중간 지점 — 오픈소스 sky130으로 GDS까지 전체 플로우를 실제로 돌려 툴체인과 타이밍을 검증합니다.</p></Card>

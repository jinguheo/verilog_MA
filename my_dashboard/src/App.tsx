...
export default function App() {
  ...
  const tabs = useMemo(() => [
    ...
    { id: 'analog', label: 'Analog & Memory' }, { id: 'knowledge', label: 'Knowledge DB', count: data.sources.length }, { id: 'tools', label: 'Tool Comparison' },
  ], [data.sources.length])
  const title = page === 'pipeline' ? 'RTL Development Pipeline' : page === 'analog' ? 'Analog & Memory Design' : page === 'tools' ? 'EDA Tool Comparison' : page === 'knowledge' ? 'Knowledge DB Overview' : page === 'sample' ? 'Sample Test 1' : page === 'sample2' ? 'Sample Test 2' : page === 'sample3' ? 'Sample Test 3 — Data Acquisition Subsystem' : page === 'sample4' ? 'Sample Test 4 — Multi-channel DAQ/DMA' : 'Task Workspace'
  ...
  const content = page === 'pipeline' ? pipeline : page === 'analog' ? <AnalogDesign/> : page === 'tools' ? <ToolComparison/> : page === 'knowledge' ? knowledge : page === 'sample' ? <SampleTest/> : page === 'sample2' ? <SampleTest2/> : page === 'sample3' ? <SampleTest3/> : page === 'sample4' ? <SampleTest4/> : <TaskWorkspace taskId={page as TaskId} data={data}/>

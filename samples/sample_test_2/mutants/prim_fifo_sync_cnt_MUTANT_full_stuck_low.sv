// MUTANT COPY - NOT the real OpenTitan source. Do not use for anything but
// the checker-sanity demo referenced from samples/sample_test_2's dashboard.
//
// This is prim_fifo_sync_cnt.sv (see D:\MyWork\verilog\dbs\opentitan\hw\ip\prim\rtl)
// with exactly one line changed: full_o is stuck at 1'b0, so the FIFO never
// reports full and silently overflows past its configured Depth instead of
// rejecting writes. It exists to answer "can we make an assertion actually
// fail?" - swap this file in for the real prim_fifo_sync_cnt.sv (same module
// name, drop-in replacement) and re-run the same UVM/formal harnesses
// unchanged:
//   - UVM: fifo_driver's REQ-FIFO-002 check ("full_o not asserted at depth 4")
//     and REQ-FIFO-005 check ("push accepted while full - overflow") both
//     fire as UVM_ERROR, and the scoreboard eventually reports a data
//     mismatch once overwritten entries are popped.
//   - Formal: `full_o == (depth_o == 4)` and the no-silent-overflow property
//     in prim_fifo_sync_formal.sv both fail k-induction with a concrete
//     counterexample trace.
// Run with: run_verilator_uvm.ps1 -Mutant, or formal\prim_fifo_sync_MUTANT.sby
//
...
module prim_fifo_sync_cnt #(
  ...
  // MUTANT: full is supposed to be
  //   wptr_wrap_cnt_q == (rptr_wrap_cnt_q ^ {1'b1, {(WrapPtrW-1){1'b0}}})
  // Stuck at 0 instead, so the FIFO never blocks writes and silently
  // overflows once more than Depth entries are pushed without a pop.
  assign full_o = 1'b0;

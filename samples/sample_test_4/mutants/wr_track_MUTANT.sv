// Mutant copy of rtl/dma/wr_track.sv. See mutants/skid_buffer_MUTANT.sv for
// why these exist and what -Mutant does with them.
//
//   MUT_TRACK_NOERR    a burst's BRESP error is never latched - ch_err_o
//                       stays 0 no matter how many bursts actually errored.
//   MUT_TRACK_NOCLEAR   ch_abort_i no longer clears a latched error - once
//                       set, ch_err_o/ch_err_code_o are sticky forever.
//   MUT_TRACK_WRONGCH   xfer_done_ch_o is always channel 0, regardless of
//                       which channel's burst actually completed.
//
...
`ifdef MUT_TRACK_WRONGCH
  assign xfer_done_ch_o = '0;
`else
  assign xfer_done_ch_o = burst_done_ch_i;
`endif

  logic this_burst_errored;
`ifdef MUT_TRACK_NOERR
  assign this_burst_errored = 1'b0;
`else
...
`endif
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      for (int unsigned c = 0; c < NumCh; c++) begin
`ifdef MUT_TRACK_NOCLEAR
        if (this_burst_errored & (burst_done_ch_i == ChIdxW'(c))) begin
          err_q[c]      <= 1'b1;

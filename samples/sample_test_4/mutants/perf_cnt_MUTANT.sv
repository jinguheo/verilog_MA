// Mutant copy of rtl/stat/perf_cnt.sv. See mutants/skid_buffer_MUTANT.sv for
// why these exist and what -Mutant does with them.
//
//   MUT_PERF_BYTEWRONG  the byte counter increments by the full bus width
//                        every accepted beat, ignoring beat_strb_i's actual
//                        popcount - wrong whenever a beat is not fully
//                        enabled (a partial final beat).
//   MUT_PERF_NOSTALL    the stall condition drops its "not ready" qualifier,
//                        so CH_STALL_CNT never increments at all.
//   MUT_PERF_ERRMISS    the error-count condition omits IrqCauseCrc, so a
//                        CRC-32 mismatch is not counted as an error event.
//
...
`ifdef MUT_PERF_NOSTALL
    assign stall     = 1'b0;
`else
    assign stall     = beat_valid_i[c] & ~beat_ready_i[c];
`endif
    assign crc_cause = ch_cause_i[c][IrqCauseCrc];
`ifdef MUT_PERF_ERRMISS
    assign err_cause = ch_cause_i[c][IrqCauseErr] | ch_cause_i[c][IrqCauseFifoOvf];
`else

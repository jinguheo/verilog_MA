// Mutant copy of rtl/dma/axi_wr_master.sv. See mutants/skid_buffer_MUTANT.sv
// for why these exist and what -Mutant does with them.
//
//   MUT_WRM_NOSPLIT    burst sizing ignores both the MaxBurst cap and the
//                       4 KB boundary check - every transfer is issued as a
//                       single burst covering its whole remaining length,
//                       even one that would cross a boundary or need more
//                       beats than AXI4's own 256-beat limit allows.
//   MUT_WRM_LASTWRONG  wlast_o is asserted on every beat of a burst, not
//                       just its final one.
//   MUT_WRM_ERRDROP    burst_done_err_o is always 0 - a BRESP error on any
//                       burst is silently dropped instead of surfaced.
//
...
// protocol/semantics split desc_fetch.sv's header describes between itself
// and axi_rd_master.
...
(
  ...
  // Sized to BeatCntW - see the golden RTL's own comment on why. Under
  // MUT_WRM_NOSPLIT this truncates remain_beats_q down to its low 5 bits,
  // which is exactly the defect: a transfer longer than 32 beats gets a
  // wrapped, wrong AWLEN instead of being capped/split at all.
  logic [BeatCntW-1:0] burst_beats_c;
`ifdef MUT_WRM_NOSPLIT
  assign burst_beats_c = BeatCntW'(remain_beats_q);
`else
...
`endif
  ...
  // burst_beats_q - see the golden RTL's own comment on why.
...
`ifdef MUT_WRM_LASTWRONG
  assign wlast_o   = w_active;
`else
...
`ifdef MUT_WRM_ERRDROP
  assign burst_done_err_o   = 1'b0;
`else

module prim_filter_tb;
  ...
  endtask
  initial begin
    // Generate a real falling reset edge after initialization so the
    // asynchronous-reset flops are deterministically initialized.
    #1 rst_ni = 1;
    #1 rst_ni = 0;
    repeat (2) @(posedge clk_i); rst_ni = 1; @(posedge clk_i);
    filter_i = 1; enable_i = 0; check_value(1, "bypass follows input");
    // Reapply reset: the sample history is intentionally collected even
    // while bypassed, so a clean filtering window starts after reset.
    rst_ni = 0; #1; rst_ni = 1;
    filter_i = 1; enable_i = 1; check_value(0, "enabled output holds reset value initially");
